...
{
    auto sub =
        m.def_submodule("flatness", "Forward and backward flatness transforms");
    py::class_<minco::flatness::FlatnessMap>(sub, "FlatnessMap")
        ...
        .def("configure_from_file",
             ...
             "Load flatness parameters from a YAML configuration file (empty "
             "path selects the default bundle).")
        .def(
            "forward",
            [](minco::flatness::FlatnessMap &self, const Eigen::Vector3d &vel,
            ...
            py::arg("vel"), py::arg("acc"), py::arg("jer"), py::arg("psi"),
            py::arg("dpsi"),
            "Run the forward flatness map and return (thrust, quaternion, "
            "body_rates).")
        .def(
            "backward",
               ...
               const Eigen::Vector3d &omg_grad)
            {
                ...
                return py::make_tuple(pos_total_grad, vel_total_grad,
                                      acc_total_grad, jer_total_grad,
                                      psi_total_grad, dpsi_total_grad);
            },
            py::arg("pos_grad"), py::arg("vel_grad"), py::arg("thr_grad"),
            py::arg("quat_grad"), py::arg("omg_grad"),
            "Run the adjoint flatness map and return gradients with respect to "
            "flat outputs.");

    py::class_<minco::flatness::CasadiQuadrotorFlatnessMap>(
        sub, "CasadiQuadrotorFlatnessMap")
        .def(py::init<>())
        .def("configure_from_file",
             &minco::flatness::CasadiQuadrotorFlatnessMap::configure_from_file,
             py::arg("file_path") = std::string(),
             "CasADi flatness maps are generated ahead-of-time; pass an empty "
             "path or regenerate code with new parameters.")
        .def(
            "forward",
               ...
               const Eigen::Vector3d &jer, double psi, double dpsi)
            {
                ...
                return py::make_tuple(result.thrust, result.quaternion,
                                      result.angular_velocity);
            },
            py::arg("vel"), py::arg("acc"), py::arg("jer"), py::arg("psi"),
            py::arg("dpsi"),
            "Run the CasADi flatness map and return (thrust, quaternion, "
            "body_rates).")
        .def(
            "backward",
            [](minco::flatness::CasadiQuadrotorFlatnessMap &self,
               ...
               const Eigen::Vector3d &omg_grad)
            {
                const auto result = self.backward(
            ...
            py::arg("pos_grad"), py::arg("vel_grad"), py::arg("thr_grad"),
            py::arg("quat_grad"), py::arg("omg_grad"),
            "Run the CasADi adjoint map and return gradients with respect to "
            "flat outputs.");
}
