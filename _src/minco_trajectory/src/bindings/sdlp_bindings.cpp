...
{
    template <int Dim>
    std::pair<double, Eigen::VectorXd> linprog_fixed_dim(
        ...
        const Eigen::VectorXd &b)
    {
        ...
        const double                             minimum =
            sdlp::linprog<Dim>(c_fixed, A_fixed, b_fixed, x_fixed);
...
{
    auto sub =
        m.def_submodule("sdlp", "Small-dimensional linear programming helpers");
    sub.def(
        "linprog",
        [](const Eigen::VectorXd &c, const Eigen::MatrixXd &A,
           const Eigen::VectorXd &b)
        {
            if (A.cols() != c.size())
            {
                throw py::value_error(
                    "Coefficient vector dimension must match A columns");
            }
            if (b.size() != A.rows())
            {
                throw py::value_error(
                    "Right-hand side dimension must match A rows");
            }
            ...
            switch (dim)
            {
                ...
                default:
                    throw py::value_error(
                        "linprog currently supports dimensions 1 through 6");
            }

            return py::make_tuple(result.first, result.second);
        },
        py::arg("c"), py::arg("A"), py::arg("b"),
        "Solve min c^T x subject to Ax <= b. Returns (minimum, argmin). "
        "Dimensions up to 6 are supported.");
}
