...
{
    auto sub = m.def_submodule("root_finder",
    ...
    sub.def("poly_val", &RootFinder::polyVal, py::arg("coeffs"), py::arg("x"),
            py::arg("numerical_stability") = true,
            "Evaluate a polynomial at x (coefficients stored highest degree "
            "first).");
    sub.def("count_roots", &RootFinder::countRoots, py::arg("coeffs"),
            py::arg("lbound"), py::arg("ubound"),
            "Count distinct real roots inside (lbound, ubound).");
    sub.def(
        "solve_polynomial",
        [](const Eigen::VectorXd &coeffs, double lbound, double ubound,
           double tol, bool isolation)
        {
            auto roots = RootFinder::solvePolynomial(coeffs, lbound, ubound,
                                                     tol, isolation);
            return std::vector<double>(roots.begin(), roots.end());
        },
        py::arg("coeffs"), py::arg("lbound"), py::arg("ubound"), py::arg("tol"),
        py::arg("isolation") = true,
