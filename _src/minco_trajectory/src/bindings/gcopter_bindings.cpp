...
void bind_gcopter(py::module_ &m)
{
    auto sub = m.def_submodule(
        "gcopter", "Trajectory optimization helpers built on GCOPTER");

    using DefaultOptimizer = gcopter::GCOPTER_PolytopeSFC<>;
    using CasadiOptimizer  = gcopter::GCOPTER_PolytopeSFC<
        minco::flatness::CasadiQuadrotorFlatnessMap>;
    ...
    py::class_<DefaultOptimizer>(sub, "GCOPTERPolytopeSFC")
        ...
        .def(
            ...
            "Load GCOPTER parameters (flatness, cost, and solver tuning) from "
            "a YAML configuration file.")
        .def("setup_basic_trajectory",
             &DefaultOptimizer::setup_basic_trajectory, py::arg("initial_pva"),
             py::arg("terminal_pva"), py::arg("initial_time"),
             py::arg("initial_points"), py::arg("sfc_control_points"),
             py::arg("smoothing_factor"), py::arg("integral_resolution"))
        .def(
            "optimize",
    ...
    py::class_<CasadiOptimizer>(sub, "GCOPTERPolytopeSFCCasadi")
        ...
        .def(
            ...
            { self.configure_from_file(file_path); },
            py::arg("file_path") = std::string(),
            "Load GCOPTER parameters when using the CasADi quadrotor flatness "
            "map. "
            "Flatness parameters are embedded at build time; re-run the CasADi "
            "code "
            "generation tool to update them.")
        .def("setup_basic_trajectory", &CasadiOptimizer::setup_basic_trajectory,
