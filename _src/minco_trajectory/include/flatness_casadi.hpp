    ...
    class CasadiQuadrotorFlatnessMap
    {
       public:
        ...
        CasadiQuadrotorFlatnessMap()
            : config_(casadi_generated::kEmbeddedConfig)
        {
        }
        ...
        inline void configure(const ConfigType &config)
        {
            ...
            throw std::logic_error(
                "CasADi flatness map embeds parameters at code generation "
                "time. "
                "Run `python -m minco.flatness_cache --generate-c` to "
                            ...
                            Eigen::Vector4d &quat, Eigen::Vector3d &omg)
        {
            ...
            std::array<const double *, 5> args{
                vel.data(), acc.data(), jer.data(), &psi, &dpsi,
            };
            ...
            check_status(casadi_quadrotor_flatness_forward(
                             args.data(), res.data(), iw.data(), w.data(), 0),
            ...
            double &dpsi_total_grad) const
        {
            ...
            {
                throw std::logic_error(
                    "CasADi flatness backward requires a prior forward "
                    "evaluation.");
            }
            ...
            std::array<const double *, 10> args{
                state.velocity.data(), state.acceleration.data(),
                state.jerk.data(),     &state.yaw,
                &state.yaw_rate,       pos_grad.data(),
                vel_grad.data(),       &thr_grad,
                quat_grad.data(),      omg_grad.data(),
            };
            ...
            check_status(casadi_quadrotor_flatness_backward(
                             args.data(), res.data(), iw.data(), w.data(), 0),
                         "casadi_quadrotor_flatness_backward");

            pos_total_grad = Eigen::Map<const Eigen::Vector3d>(buffer.data());
            vel_total_grad =
                Eigen::Map<const Eigen::Vector3d>(buffer.data() + 3);
            acc_total_grad =
                Eigen::Map<const Eigen::Vector3d>(buffer.data() + 6);
            jer_total_grad =
                Eigen::Map<const Eigen::Vector3d>(buffer.data() + 9);
       ...
       private:
        ...
        static inline const std::array<casadi_int, 4> &
        backward_workspace_sizes()
        {
            static const std::array<casadi_int, 4> sizes = []
            {
                casadi_int arg = 0;
