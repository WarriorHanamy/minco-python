...
namespace lbfgs
{
    ...
    struct lbfgs_parameter_t
    {
         ...
         *  not directly reduce first-order residual. It still needs the
         * function value which can be corrupted by machine_prec when ||g|| is
         * small.
         */
         ...
         *  The lbfgs_optimize() function terminates an minimization process
         * with
         ...
         *  the exponents are too large for the machine being used, or unless
        ...
        /**
         * The machine precision for floating-point values. The default
         * is 1.0e-16. This parameter must be a positive value set by a client
         * program to estimate the machine precision.
         */
    ...
    enum
    {
        ...
        /** Line search reaches the maximum, assumptions not satisfied or
     ...
     * cost function and its gradients when needed. A client program must
     * implement this function to evaluate the values of the cost function and
     * its gradients, given current values of variables.
     *
     *  @param  instance    The user data sent for lbfgs_optimize() function by
     * the client.
     *  @param  x           The current values of variables.
     *  @param  g           The gradient vector. The callback function must
     * compute the gradient values for the current variables.
    ...
    /**
     * Callback interface to provide an upper bound at the beginning of the
     * current line search.
     *
     *  The lbfgs_optimize() function call this function to obtain the values of
     * the upperbound of the stepsize to search in, provided with the beginning
     * values of variables before the line search, and the current step vector
     * (can be descent direction). A client program can implement this function
     * for more efficient linesearch. Any step larger than this bound should not
     * be considered. For example, it has a very large or even inf function
     * value. Note that the function value at the provided bound should be
     * FINITE! If it is not used, just set it nullptr.
     *
     *  @param  instance    The user data sent for lbfgs_optimize() function by
     * the client.
     ...
     * current progress of the minimization process. If it is not used, just set
     * it nullptr.
     *
     *  @param  instance    The user data sent for lbfgs_optimize() function by
     * the client.
     ...
     *  @retval int         Zero to continue the minimization process. Returning
     * a non-zero value will cancel the minimization process.
     */
     ...
     * (e.g., variables, function, and gradient, etc) and to cancel the
     * iteration process if necessary. Implementation of the stepbound and the
     * progress callback is optional: a user can pass nullptr if progress
     * notification is not necessary.
     *
     *
     *  @param  x               The vector of decision variables.
     *                          THE INITIAL GUESS x0 SHOULD BE SET BEFORE THE
     * CALL! A client program can receive decision variables through this
     * vector, at which the cost and its gradient are queried during
     * minimization.
     *  @param  f               The ref to the variable that receives the final
     *                          value of the cost function for the variables.
     *  @param  proc_evaluate   The callback function to provide function f(x)
     * and gradient g(x) evaluations given a current values of variables x. A
     * client program must implement a callback function compatible with
     ...
     *                          upperbound of the stepsize to search in,
     * provided with the beginning values of variables before the line search,
     * and the current step vector (can be negative gradient). A client program
     * can implement this function for more efficient linesearch. If it is not
     ...
     *  @retval int             The status code. This function returns a
     * nonnegative integer if the minimization process terminates without an
     * error. A negative integer indicates an error.
     */
            ...
            */
            ...
            while (true)
            {
                ...
                if (cd.proc_stepbound)
                {
                    ...
                    step_max =
                        step_max < param.max_step ? step_max : param.max_step;
                ...
                */
                ...
                1054-1064, 2011.
                */
                cau = lm_s.col(end).squaredNorm() * gp.norm() *
                      param.cautious_factor;
                ...
                if (ys > cau)
                {
                    ...
                    */
                    ...
                    for (i = 0; i < bound; ++i)
                    {
                        ...
                        /* \gamm_{i+1} = \gamm_{i} + (\alpha_{j} - \beta_{j})
                         * s_{j}. */
    ...
    inline const char *lbfgs_strerror(const int err)
    {
        switch (err)
        {
            ...
            case LBFGS_STOP:
                return "Success: met stopping criteria (past f decrease less "
                       "than delta).";

            case LBFGS_CANCELED:
                return "The iteration has been canceled by the monitor "
                       "callback.";
            ...
            case LBFGSERR_INVALID_MEMSIZE:
                return "Invalid parameter lbfgs_parameter_t::mem_size "
                       "specified.";

            case LBFGSERR_INVALID_GEPSILON:
                return "Invalid parameter lbfgs_parameter_t::g_epsilon "
                       "specified.";
            ...
            case LBFGSERR_INVALID_MINSTEP:
                return "Invalid parameter lbfgs_parameter_t::min_step "
                       "specified.";

            case LBFGSERR_INVALID_MAXSTEP:
                return "Invalid parameter lbfgs_parameter_t::max_step "
                       "specified.";

            case LBFGSERR_INVALID_FDECCOEFF:
                return "Invalid parameter lbfgs_parameter_t::f_dec_coeff "
                       "specified.";

            case LBFGSERR_INVALID_SCURVCOEFF:
                return "Invalid parameter lbfgs_parameter_t::s_curv_coeff "
                       "specified.";

            case LBFGSERR_INVALID_MACHINEPREC:
                return "Invalid parameter lbfgs_parameter_t::machine_prec "
                       "specified.";
            ...
            case LBFGSERR_MAXIMUMLINESEARCH:
                return "Line search reaches the maximum try number, "
                       "assumptions not satisfied or precision not achievable.";
            ...
            case LBFGSERR_INCREASEGRADIENT:
                return "The current search direction increases the cost "
                       "function value.";
